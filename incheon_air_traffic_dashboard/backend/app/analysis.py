...
def linear_forecast(records: List[Tuple[dt.datetime, float]], last_ts: dt.datetime,
    ...
    warn = ""
    if n <= 1:
        warn = (f" ⚠ 구간 일수가 {n}일뿐이라 기울기(추세)를 통계적으로 추정할 수 없습니다 — "
                f"예측값은 해당 기간 평균을 그대로 연장한 것입니다. 최소 2일 이상이어야 선이 그려지고, "
                f"신뢰할 만한 추세를 보려면 5~7일 이상을 권장합니다.")
    elif n < 5:
        warn = f" ⚠ 표본이 {n}일로 적어 추세선이 며칠간의 우연한 변동에 민감할 수 있습니다. 최소 5~7일 이상 구간을 권장합니다."

    note = (f"선택 구간(일평균 {n}개 지점)의 선형회귀 기울기는 하루당 {'+' if b>=0 else ''}{b:.2f}㎍/㎥ 입니다. "
            f"이 추세를 향후 {horizon_days}일 연장한 예측입니다.{warn}")
