...
with st.sidebar:
    ...
    uploaded_files = st.file_uploader("규정 파일 (PDF, DOCX)", type=["pdf", "docx", "hwp"], accept_multiple_files=True)
...
if os.path.exists(PERSIST_DIRECTORY):
    try:
        ...
        if doc_data.get("documents"):
            ...
            st.success("✅ 하이브리드 검색 준비 완료 (BM25 + Vector)")
        else:
...
if prompt := st.chat_input("질문하세요..."):
    ...
    with st.chat_message("assistant"):
        if chroma_retriever:
            # Windows 속도 개선을 위해 keep_alive 옵션 추가 고려 가능 (현재는 기본)
            llm = ChatOllama(
